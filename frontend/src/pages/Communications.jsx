...
export default function Communications() {
  ...
  async function toggleConnect(intg) {
    setConnecting(intg.integration_type)
    try {
      if (intg.status === 'connected') {
        await api.delete(`/api/integrations/${intg.id}`)
      } else {
        await api.post(`/api/integrations/${intg.id}/connect`, {})
      }
      refetchIntg()
    } finally {
      setConnecting(null)
            ...
            return (
              <div key={intg.id} className={`p-3 border rounded-lg ${connected ? 'border-green-200 bg-green-50' : 'border-gray-200'}`}>
                ...
                <button
                  ...
                  disabled={connecting === intg.integration_type}
                ...
                >
                  {connecting === intg.integration_type ? '処理中...' : connected ? '接続解除' : '接続する'}
                </button>
              </div>
            )
          })}
